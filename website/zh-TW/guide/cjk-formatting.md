...
當 [MCP 伺服器](/guide/mcp-setup)已連線時，AI 助手可以透過 `cjk_format` 動作以程式方式套用中日韓文格式化。

**範圍選項：**
- `"document"`（預設）——使用你的中日韓文設定格式化整份文件
- `"selection"`——僅格式化目前選取的文字

兩種範圍都使用序列化-格式化-解析來回傳遞來保留行內標記（粗體、連結、數學公式等）並遵循你設定的格式化規則。

有關中日韓文相關 MCP 動作的完整清單，包括 `cjk_punctuation` 和 `cjk_spacing`，請參閱 [MCP 工具參考](/guide/mcp-tools)。
