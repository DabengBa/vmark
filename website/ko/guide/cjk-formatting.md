...
[MCP 서버](/guide/mcp-setup)가 연결되어 있으면 AI 어시스턴트가 `cjk_format` 액션을 통해 프로그래밍 방식으로 CJK 서식을 적용할 수 있습니다.

**범위 옵션:**
- `"document"` (기본값) — CJK 설정을 사용하여 전체 문서를 서식화
- `"selection"` — 현재 선택된 텍스트만 서식화

두 범위 모두 인라인 마크 (굵게, 링크, 수식 등)를 보존하고 구성된 서식 규칙을 준수하기 위해 직렬화-서식-파싱 왕복을 사용합니다.

`cjk_punctuation` 및 `cjk_spacing`을 포함한 CJK 관련 MCP 액션의 전체 목록은 [MCP 도구 참조](/guide/mcp-tools)를 확인하세요.
