...
当 [MCP 服务器](/guide/mcp-setup)已连接时，AI 助手可以通过 `cjk_format` 操作以编程方式应用中日韩格式化。

**范围选项：**
- `"document"`（默认）——使用你的中日韩设置格式化整个文档
- `"selection"`——仅格式化当前选中的文本

两种范围都使用序列化-格式化-解析往返来保留内联标记（粗体、链接、数学公式等）并遵循你配置的格式化规则。

有关中日韩相关 MCP 操作的完整列表，包括 `cjk_punctuation` 和 `cjk_spacing`，请参阅 [MCP 工具参考](/guide/mcp-tools)。
