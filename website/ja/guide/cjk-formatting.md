...
[MCP サーバー](/guide/mcp-setup)が接続されている場合、AI アシスタントは `cjk_format` アクションを通じてプログラム的に CJK フォーマットを適用できます。

**スコープオプション:**
- `"document"`（デフォルト）— CJK 設定を使用してドキュメント全体をフォーマット
- `"selection"` — 現在選択されているテキストのみをフォーマット

両方のスコープは、インラインマーク（太字、リンク、数式など）を保持し、設定されたフォーマットルールを尊重するために、シリアライズ-フォーマット-パースのラウンドトリップを使用します。

`cjk_punctuation` や `cjk_spacing` を含む CJK 関連の MCP アクションの完全なリストは、[MCP ツールリファレンス](/guide/mcp-tools)を参照してください。
