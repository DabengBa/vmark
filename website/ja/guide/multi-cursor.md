...
VMarkはWYSIWYGモードとソースモードの両方で強力なマルチカーソル編集をサポートし、複数の場所を同時に編集できます。
...
`Alt`（macOSではOption）を押しながらクリックして:
- その位置に**カーソルを追加**する
...
2. 1つ多く追加してしまった場合、`Alt + Mod + Z`を押す
3. 最後に追加されたカーソルが削除され、前の状態に戻る

`Escape`（すべてを折りたたむ）とは異なり、ソフトアンドゥは一度に1つのカーソルをステップバックします。
...
`Escape`を押してプライマリ位置の1つのカーソルに戻ります。
...
- **アトムノード**: WYSIWYGモードでは画像、埋め込みコンテンツ、数式ブロック内にカーソルを置けません
- **IME入力**: 中国語、日本語などの入力メソッドを使用する場合、変換はプライマリカーソルのみに影響します
